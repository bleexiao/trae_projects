...
# 导入anythingllm_query函数
from anythingllm_query import anythingllm_query
...
tools = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "通过AnythingLLM查询文档仓库中的信息，当用户提到'文档仓库'、'文件仓库'、'仓库'时使用此工具",
            "parameters": {
                "type": "object",
                "properties": {
                    "message": {
                        "type": "string",
                        "description": "查询消息，描述要从文档仓库中获取的信息"
                    }
                },
                "required": ["message"]
            }
...
def execute_tool_call(tool_call):
    ...
    if function_name == "list_files":
        ...
        result = anythingllm_query(arguments['message'])
...
def main():
    ...
    print("当提到'文档仓库'、'文件仓库'、'仓库'时，将调用AnythingLLM查询")
    print("=====================================\n")
...
6. anythingllm_query(message) - 通过AnythingLLM查询文档仓库中的信息，当用户提到'文档仓库'、'文件仓库'、'仓库'时使用此工具

当用户请求需要使用这些工具时，请调用相应的工具并根据工具返回的结果为用户提供回答。对于read_file返回的内容摘要，请基于摘要为用户提供总结性的回答，而不是直接照搬原文。当用户提到'文档仓库'、'文件仓库'、'仓库'时，应调用anythingllm_query工具来获取相关信息。"""
